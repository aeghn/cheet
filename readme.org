...
** Add
   ...
   You can add more than one entry at a time, and starting from the first non-empty or %%.*%% line, every four lines are a group, which are the program(command), argument(keybinding), description and notes in order.
